...
import BbvaSummary from "./summaries/BbvaSummary";
import TransactionTable from "./tables/TransactionTable";
import { Wallet, Globe } from "lucide-react";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";

interface BbvaAnalysisProps {
  data: AnalysisResponse;
}

const BbvaAnalysis = ({ data }: BbvaAnalysisProps) => {
  // Para Banamex, siempre tomamos la primera (y única) cuenta del arreglo.
  const account = data.cuentas[0];

  if (!account) {
    return <div>No se encontraron datos de la cuenta para BBVA.</div>;
  }
