...
import BanbajioSummary from "./summaries/BanbajioSummary";
import TransactionTable from "./tables/TransactionTable";

interface BanbajioAnalysisProps {
  data: AnalysisResponse;
}

const BanbajioAnalysis = ({ data }: BanbajioAnalysisProps) => {
  return (
      ...
      {data.cuentas.map((account, index) => (
        <Card key={index} className="shadow-lg border-primary/20">
          ...
          <CardContent className="flex flex-col gap-8">
            ...
            <BanbajioSummary account={account} />
...
export default BanbajioAnalysis;